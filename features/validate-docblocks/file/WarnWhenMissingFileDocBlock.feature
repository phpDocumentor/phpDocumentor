Feature: Validate whether the File-level DocBlock is valid
  ...
  Scenario: Show an error when no File-level DocBlock is present but a Class DocBlock is first
  ...
  Scenario: Show an error when a File-level DocBlock is present, it has no package tag and is not preceeding a class DocBlock
    Given I am in the phpDocumentor root directory
  ...
  Scenario: Show an error when a DocBlock is first in a file but directly preceeding a documentable element (i.e. define)
    Given I am in the phpDocumentor root directory
