...
function profiler_report (
    ...
    $base_url_params = xhprof_array_unset(
        xhprof_array_unset($url_params, 'symbol'),
        'all'
    );
    ...
    if ($diff_mode) {
        ...
        $run1_link = xhprof_render_link(
            'View Run #' . $run1,
            "$base_path/?" . http_build_query(
                xhprof_array_set($base_url_params, 'run', $run1)
            )
        );
        $run2_txt = sprintf(
            "<b>Run #%s:</b> %s",
        ...
        $run2_link = xhprof_render_link(
            'View Run #' . $run2,
            "$base_path/?" . http_build_query(
                xhprof_array_set($base_url_params, 'run', $run2)
            )
        );
    } else {
        $diff_text = "Run";
...
function print_function_info($url_params, $info, $sort, $run1, $run2)
{
    ...
    $href = "$base_path/?" .
        http_build_query(
            xhprof_array_set($url_params, 'symbol', $info["fn"])
        );
...
function print_flat_data(
    ...
    if (!$limit) {              // no limit
        ...
        $display_link = xhprof_render_link(
            ...
            "$base_path/?" . http_build_query(
                xhprof_array_set($url_params, 'all', 1)
            )
        );
    }
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2)
{
    ...
    if ($diff_mode) {
        ...
        $base_url_params = xhprof_array_unset(
            xhprof_array_unset($url_params, 'run1'),
            'run2'
        );
        $href1 = "$base_path/?" .
          http_build_query(
...
function symbol_report(
    ...
    foreach ($pc_stats as $stat) {
        ...
        if (array_key_exists($stat, $sortable_columns)) {
            ...
            $href = "$base_path/?" .
                http_build_query(
                    xhprof_array_set($url_params, 'sort', $stat)
                );
            $header = xhprof_render_link($desc, $href);
        } else {
            $header = $desc;
