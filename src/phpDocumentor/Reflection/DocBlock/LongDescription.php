 ...
 * @link     http://phpdoc.org
 */
class phpDocumentor_Reflection_DocBlock_LongDescription implements Reflector
{
    /** @var string */
    protected $contents = '';

    /** @var phpDocumentor_Reflection_DocBlock_Tags[] */
    ...
    public function __construct($content)
    {
        if (preg_match('/\{\@(.+?)\}/', $content, $matches)) {
            ...
            foreach ($matches as $tag) {
                $this->tags[] = phpDocumentor_Reflection_DocBlock_Tag::createInstance(
                    '@' . $tag
                );
            }
        }
